...
class KimiK3MLAAttention(DeepseekV2AttentionMLA):
    ...
    def __init__(
        ...
        if _is_hip:
            ...
            # Fuse the q/k RMSNorm with q_b_proj's per-token quant.
    ...
    def _fork_output_gate(self, hidden_states: torch.Tensor) -> None:
        ...
        if (
            ...
            and not is_in_breakable_cuda_graph()
            and (0 < _k3_hidden_num_tokens(hidden_states) <= self._gate_bs_limit)
        ):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.use_output_gate:
            # A fused in-proj quant already stashed the gate input.
...
class KimiK3DecoderLayer(nn.Module):
    ...
    def _forward_attn_residual(
        ...
        # ROCm decode: leave the out-norm to a fused RMSNorm+quant producer.
        inproj = _k3_attn_inproj(self.self_attn)
        skip_out_norm = (
            not input_sharded
