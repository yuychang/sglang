...
class ServerArgs:
    ...
    mla_runner_backend: A[
        str,
        Arg(
            help=(
                "Choose the runner backend for aiter MLA kernels on ROCm. "
                "'auto' keeps the legacy Triton MLA path. "
                "'gluon' selects aiter's native Gluon MLA kernel (mla_gluon), "
                "which tiles query heads and serves non-power-of-2 counts such "
                "as Kimi-K3's 12 local heads at tp8."
            ),
            choices=MLA_RUNNER_BACKEND_CHOICES,
            resolvable=True,
        ),
        NS("exec.kernel"),
    ] = "auto"
    ...
    def _handle_dcp_validation(self):
        ...
        if self.mla_runner_backend == "gluon":
            if not is_hip():
                raise ValueError(
                    "--mla-runner-backend gluon is only supported on ROCm."
                )
            if not self.use_mla_backend():
                raise ValueError(
                    "--mla-runner-backend gluon requires an MLA model."
                )

    def _validate_aiter_mla_dcp(self):
        """Validate aiter MLA decode-context-parallel (DCP).

        When --mla-runner-backend gluon (or the Kimi-K3 DCP override),
        the decode path runs aiter's Gluon MLA kernel over each rank's
        round-robin KV shard (see AiterAttnBackend._mla_decode_fwd_dcp_gluon).
        Gluon tiles the query heads, so it serves any gathered head count
        (Kimi-K3's 96 at tp8 dcp8 works) and native 12 local heads at tp8.
        """
    ...
    def _get_default_attn_backend(self, use_mla_backend: bool, model_config):
        ...
        if not use_mla_backend:
            ...
            if is_hopper_with_cuda_12_3():
                ...
                head_num = model_config.get_num_attention_heads(self.tp_size)
                if self.mla_runner_backend == "gluon" and head_num <= 96:
                    return "aiter"
                # Legacy Triton MLA path only supports 16 or 128 local query heads.
    ...
    def use_aiter_gluon_mla(self) -> bool:
        """Whether aiter MLA should run on the native Gluon kernel (mla_gluon)."""
        if self.mla_runner_backend == "gluon":
            return True
        if self.mla_runner_backend == "triton":
            return False
        return envs.SGLANG_USE_AITER_GLUON_MLA_DCP.get()
