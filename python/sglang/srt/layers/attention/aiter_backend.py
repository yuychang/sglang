...
class AiterAttnBackend(AttentionBackend):
    ...
    def __init__(
        ...
        if self.use_mla:
            ...
            assert _valid_heads or not may_run_mla_decode, (
    ...
    def _mla_decode_fwd_with_head_pad(
        ...
        **kwargs,
        ...
        zero (12): pad four zero-valued dummy heads to 16.
        ...
        if self.head_pad_mode == "zero":
            q_in = q.new_zeros(
                (q.shape[0], self.num_head_padded, q.shape[-1]), dtype=q.dtype
            )
            q_in[:, :num_head, :] = q
            o = q.new_empty(
                (q.shape[0], self.num_head_padded, layer.v_head_dim),
            ...
            return o[:, :num_head, :]
        o = q.new_empty(
            (q.shape[0], num_head, layer.v_head_dim), dtype=self.input_dtype
        )
        mla_decode_fwd(q, k_buffer_flat, o, **kwargs)
        return o

    def _zero_pad_mla_q_heads(self, q: torch.Tensor, layer) -> torch.Tensor:
    ...
    def _resolve_fp8_kv_scale_float(self, layer, k_descale) -> float:
