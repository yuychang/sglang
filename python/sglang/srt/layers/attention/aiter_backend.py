...
# mla_gluon's bf16 decode regime (bh16bn64) walks the KV in tiles of BLOCK_N=64
# and blocks of BLOCK_H=16 query heads. Both are baked into the kernel; they are
# repeated here only to size NUM_KV_SPLITS the way the wrapper does.
_GLUON_MLA_BLOCK_N = 64
_GLUON_MLA_BLOCK_H = 16
# Above 96 heads mla_gluon leaves the head-tiling regime for bh64, which wants a
# batch that is a multiple of 64 and a minimum sequence length -- neither of
# which a decode batch can promise.
_GLUON_MLA_MAX_HEADS = 96
# mla_gluon aims its split count at one full wave of workgroups on MI350/MI355.
_GLUON_MLA_TARGET_WGS = 256


def gluon_mla_min_kv_seq_len(bs: int, num_heads: int, max_context_len: int) -> int:
    """The ``min_kv_seq_len`` that gives mla_gluon a stable KV split count.

    mla_gluon does not take NUM_KV_SPLITS directly. It derives it as
    ``min(256 // (bs * qlen * cdiv(nhead, 16)), cdiv(min_kv_seq_len, 64))`` and
    passes it as a constexpr, so the value is baked into both the launch grid
    and the compiled kernel. Handing it the batch's real KV length would
    therefore reshape the grid from step to step -- which a captured cuda graph
    cannot express -- and would recompile the kernel for every distinct length.

    So ask instead for the largest power-of-two split count the workgroup budget
    allows. It depends only on (bs, nhead), both fixed per captured graph, and
    rounding to a power of two keeps the number of compiled variants in single
    digits. Splits a short sequence cannot fill are not a correctness problem:
    stage 1 returns early on an empty split and stage 2 skips it.
    """
    head_blocks = triton.cdiv(num_heads, _GLUON_MLA_BLOCK_H)
    budget = max(1, _GLUON_MLA_TARGET_WGS // max(bs * head_blocks, 1))
    splits = 1 << (budget.bit_length() - 1)
    # Never ask for more splits than the longest sequence has KV tiles.
    splits = min(splits, max(1, triton.cdiv(max_context_len, _GLUON_MLA_BLOCK_N)))
    return splits * _GLUON_MLA_BLOCK_N
...
class AiterAttnBackend(AttentionBackend):
    ...
    def __init__(
        ...
        if self.use_mla:
            ...
            # Without DCP, --mla-runner-backend gluon runs decode on
            # _mla_decode_fwd_gluon, whose head tiling lifts the {4, 8, 16k}
            # restriction up to the 96 heads mla_gluon's bh16 regime covers.
            _gluon_serves_heads = (
                self.use_aiter_gluon_mla and self.num_head <= _GLUON_MLA_MAX_HEADS
            )
            assert _valid_heads or _gluon_serves_heads, (
    ...
    def _mla_decode_fwd_gluon(self, q, layer):
        """MLA decode on aiter's Gluon kernel, without decode context parallel.

        Selected by ``--mla-runner-backend gluon``. The reason to prefer it over
        aiter's legacy MLA decode is head tiling: mla_gluon splits the query
        heads into blocks of 16 and masks the tail, so it serves any count up to
        96, while ``mla_decode_fwd`` only accepts 4, 8, or a multiple of 16.
        Kimi-K3's 12 local heads at tp8 are exactly the case the legacy kernel
        turns away.

        The kernel pages at a single token, so it reads the ragged token-level
        ``(kv_indices, kv_indptr)`` this backend already builds for decode and
        needs neither a paged block table nor ``kv_last_page_len``.
        """
    ...
    def _mla_decode_fwd_dcp(self, q, k_buffer, layer, k_descale):
        ...
        * ``_mla_decode_fwd_dcp_gluon`` -- aiter's Gluon MLA kernel
          (``aiter.ops.triton.gluon.mla_gluon``), selected by
          ``--mla-runner-backend gluon`` because it needs triton >= 3.7.
    ...
    def forward_decode(
        ...
        if self.use_mla:
            ...
            if self.use_aiter_gluon_mla:
                # Head-tiling decode kernel; the only one that serves a local
                # head count the legacy MLA kernel rejects (Kimi-K3 at tp8).
                # Same (tokens, heads, v_head_dim) contract as the legacy path.
                return self._mla_decode_fwd_gluon(q, layer)
