...
def _batched_gemm_a16wfp4_flatten_mxfp4_quant_kernel(
    ...
    num_k_iter: tl.constexpr = tl.cdiv(
        SPLITK_BLOCK_SIZE // 2, BLOCK_SIZE_K // 2
    )
    ...
    num_quant_blocks: tl.constexpr = (
        BLOCK_SIZE_N // _MXFP4_QUANT_BLOCK_SIZE
    )
    offs_scale_n = (
...
def _batched_gemm_a16wfp4_flatten_mxfp4_quant(
    ...
    if config is None:
        raise RuntimeError("Unsupported shape or AITER config for fused MLA value BMM")
...
def batched_gemm_a16wfp4_flatten_mxfp4_quant(
    ...
    if not can_fuse_mla_value_bmm_mxfp4_quant(x, w, w_scales):
        raise ValueError(
            ...
            "batched dimensions, and a non-split AITER GEMM config"
        )
    ...
    out_scales = torch.empty(
        (m, batch * n // _MXFP4_QUANT_BLOCK_SIZE),
        dtype=torch.uint8,
        device=x.device,
    )
