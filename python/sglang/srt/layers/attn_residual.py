...
def can_skip_out_norm(hidden_size: int, nvb: int) -> bool:
    """Whether ``forward(skip_out_norm=True)`` really skips the out-norm.

    Only ``_agg_kernel`` accepts a null out-norm; the TMA path does not, so a
    caller it serves must not plan on a fused RMSNorm+quant consumer.
    """
    return is_hip() and (nvb == 0 or _use_hip_fused(hidden_size, nvb))
...
class AttnResidual:
    ...
    def forward(
        ...
        skip_out_norm leaves the mixture unnormalized for a fused
        RMSNorm+quant consumer; honored only where can_skip_out_norm says so."""
        nvb = self.num_valid_blocks
        applied_out_norm = (
            None
            if skip_out_norm and can_skip_out_norm(hidden_states.shape[1], nvb)
            else out_norm
        )
