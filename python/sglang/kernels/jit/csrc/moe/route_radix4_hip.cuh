...
// AITER's wave64 grouped_topk has a deterministic, non-ID-order tie priority.
// Preserve that serving contract when several experts have the exact pivot key.
static __device__ __constant__ uint8_t kAiterTieLaneRank[64] = {
    ...
    45, 44, 36, 37, 38, 39, 35, 34, 33, 32, 28, 29, 30, 31, 27, 26, 25, 24, 16, 17, 18, 19,
    20, 21, 22, 23, 8,  9,  10, 11, 15, 14, 13, 12, 4,  5,  6,  7,  3,  2,  1,  0,
};
...
SGL_DEVICE uint32_t aiter_tie_priority(int expert) {
...
#pragma unroll
  for (int i = 0; i < VPT; ++i) {
    ...
    if (e < EXPERTS) {
      ...
      const float ranked = g + radix4::load_score(sbias, e);
      sig[i] = g;
      // Match AITER: NaN routing values must not displace finite experts.
      key[i] = __builtin_isnan(ranked) ? 0u : radix4::sortable(ranked);
      or_all |= key[i];
      and_all &= key[i];
    } else {
      sig[i] = 0.0f;
...
#pragma unroll
  ...
  // An early exit has exactly the required number of prefix survivors, so the
  // fast unordered compaction is sufficient. Only a full-key tie needs AITER's
  // deterministic wave64 priority.
  __syncthreads();
  if (!exact_tie) {
#pragma unroll
    for (int i = 0; i < VPT; ++i) {
...
#pragma unroll
    for (int slot = 0; slot < TOPK; ++slot) {
      if (s_cnt >= TOPK) break;
      if (tid == 0) s_best = 0xffffffffu;
      __syncthreads();

#pragma unroll
      for (int i = 0; i < VPT; ++i) {
        if (((valid >> i) & 1u) && ((alive >> i) & 1u) && (key[i] & pmask) == pivot) {
          atomicMin(&s_best, radix4::aiter_tie_priority(tid + i * BLOCK));
        }
      }
      __syncthreads();
...
#pragma unroll
      for (int i = 0; i < VPT; ++i) {
        const int e = tid + i * BLOCK;
        if (((valid >> i) & 1u) && ((alive >> i) & 1u) && (key[i] & pmask) == pivot &&
            radix4::aiter_tie_priority(e) == s_best) {
          const int pos = atomicAdd(&s_cnt, 1);
          if (pos < TOPK) {
            s_w[pos] = sig[i];
            s_id[pos] = e;
