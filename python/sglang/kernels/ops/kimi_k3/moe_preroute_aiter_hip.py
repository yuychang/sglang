...
def enabled() -> bool:
    return is_hip() and envs.SGLANG_ROCM_K3_AITER_MOE_PREROUTE_FP8.get()


def cooperative_preactivated_enabled() -> bool:
    return (
        enabled()
        and envs.SGLANG_ROCM_K3_FLYDSL_SOURCE.get().lower() != "aiter"
        and envs.SGLANG_ROCM_K3_PREROUTE_PREACTIVATED_SHARED.get()
    )
