...
_CU_COUNT = 256
_DEFAULT_TUNING = (2, 2)
# (rows_per_wave, weight_cache_modifier) per captured token bucket, measured on
# MI355X with benchmark/kernels/kimi_k3/tune_kda_input_group64.py. One token
# reaches 5.31 TB/s at (2, 2) and nothing beats it; two tokens stall at 3.03
# TB/s there and reach 3.82 at (3, 1), which is 15.81 -> 12.62 us per layer.
#
# These parameters only change how output rows are distributed over waves and
# how the weight loads are cached, never the order of the K=7168 reduction: all
# sixteen (rows_per_wave, weight_cache_modifier) combinations were verified to
# produce bit-identical output at both token counts. So this is free to select
# on speed alone.
_LAUNCH_TUNING = {1: (2, 2), 2: (3, 1)}


def _legacy_launch() -> bool:
    """Force the pre-tuning configuration, for A/B against it."""
    return envs.SGLANG_K3_KDA_GROUP64_LEGACY_LAUNCH.get()
...
def _launcher(num_tokens: int):
    rows_per_wave, weight_cache_modifier = (
        _DEFAULT_TUNING
        if _legacy_launch()
        else _LAUNCH_TUNING.get(num_tokens, _DEFAULT_TUNING)
    )
