...
class TestK3MoePairAr(CustomTestCase):
    ...
    def test_all_reduce_splits_when_predicate_hits(self):
        ...
        self.assertTrue(torch.equal(latent.reshape(-1), buf[: num_tokens * moe_hidden] + 1))
    ...
    def test_all_reduce_keeps_single_collective_under_cap(self):
        ...
        self.assertTrue(torch.equal(latent, reduced[: num_tokens * moe_hidden].view(num_tokens, moe_hidden)))
        self.assertTrue(
            torch.equal(
                shared,
