...
class TestRocmMlaValueMxfp4(CustomTestCase):
    def test_fused_bmm_quant_matches_split_path(self):
        ...
        heads, n, k = 4, 128, 512
        weight = torch.randn(
            (heads * n, k), dtype=torch.bfloat16, device="cuda"
        )
        ...
        for tokens in (1, 17, 64):
            with self.subTest(tokens=tokens):
                x = torch.randn(
                    (heads, tokens, k), dtype=torch.bfloat16, device="cuda"
                )
                self.assertTrue(
                    can_fuse_mla_value_bmm_mxfp4_quant(
                ...
                expected_fp4, expected_scales = fused_flatten_mxfp4_quant(
                    bf16_output
                )

                actual_fp4, actual_scales = (
                    batched_gemm_a16wfp4_flatten_mxfp4_quant(
                        x, weight_fp4, weight_scales
                    )
                )
                torch.testing.assert_close(
                    actual_fp4, expected_fp4, rtol=0, atol=0
                )
                torch.testing.assert_close(
